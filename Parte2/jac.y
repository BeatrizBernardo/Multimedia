...
Program_2: Program_2 FieldDecl          {       if($1 != NULL){
                                                        $$ = $1; 
                                                        $$->irmao = criarIrmao($$, $2);
                                                }else{
                                                        $$ = $2;
...
FieldDecl: PUBLIC STATIC Type ID comma_id SEMI          {       $$ = criarNo("FieldDecl", NULL); 
                                                                printf("AAA<-<-<-<-<-<--<<- %s %s\n", $$->tipoVariavel, $$->valor);
                                                                $$->filho = $3;
                                                                ARVORE no;
                                                                if((no = criarNo("Id", $4))!= NULL){
                                                                        $$->irmao = criarIrmao($$, no); 
                                                                }
                                                                if($5 != NULL){
                                                                        $$->irmao = criarIrmao($$, $5);
...
MethodDecl: PUBLIC STATIC MethodHeader MethodBody       {$$ = criarNo("MethodDecl", NULL); $$->filho = $3; $$->filho = criarIrmao($$->filho, $4);}
        ;

MethodHeader: Type ID OCURV MethodHeader_2 CCURV        {$$ = criarNo("MethodHeader", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, criarNo("Id", $2)); $$->filho = criarIrmao($$->filho, $4);}
        |VOID ID OCURV MethodHeader_2 CCURV             {$$ = criarNo("MethodHeader", NULL); $$->filho = criarNo("Void", NULL); $$->filho = criarIrmao($$->filho, criarNo("Id", $2)); $$->filho = criarIrmao($$->filho, $4);}
        ;

/*deve ser opcional - sim ou não - [ FormalParams ]*/        
MethodHeader_2: FormalParams            {$$ = criarNo("MethodParams", NULL); $$->filho = criarIrmao($$->filho, criarNo("ParamDecl", NULL)); $$ = $1;}
        |                               {$$ = NULL;}
        ;
...
VarDecl: Type ID comma_id SEMI          {$$ = criarNo("VarDecl", NULL); 
                                                $$->filho = $1; 
                                                $$->filho = criarIrmao($$->filho, criarNo("Id", $2)); 
                                                printf("ola---<<<< %s %s\n", $$->filho->tipoVariavel, $$->filho->valor); 
                                                $$->filho = criarIrmao($$->filho, $3); 
                                                printf("adeus---<<<< %s %s\n", $$->filho->tipoVariavel, $$->filho->valor);}
        ;
...
Statement: OBRACE Statement_2 CBRACE                            {$$ = $2;}
        | IF OCURV Expr CCURV Statement %prec IFX               {$$ = criarNo("If", NULL); $$->filho = $3; $$->filho = criarIrmao($$->filho, $5);}
        | IF OCURV Expr CCURV Statement ELSE Statement          {$$ = criarNo("If", NULL); $$->filho = $3; $$->filho = criarIrmao($$->filho, $5); $$->filho = criarIrmao($$->filho, $7);}
        | WHILE OCURV Expr CCURV Statement                      {$$ = criarNo("While", NULL); $$->filho = $3; $$->filho = criarIrmao($$->filho, $5);}
        | DO Statement WHILE OCURV Expr CCURV SEMI              {$$ = criarNo("DoWhile", NULL); $$->filho = $2; $$->filho = criarIrmao($$->filho, $5);}
...
Assignment: ID ASSIGN Expr      {$$ = criarNo("Assign", NULL); $$->filho = criarNo("Id", $1); $$->filho = criarIrmao($$->filho, $3);}
        ;

MethodInvocation: ID OCURV CCURV                                {$$ = criarNo("Id", NULL);}
        |ID OCURV Expr MethodInvocation_2 CCURV                 {$$ = criarNo("Id", NULL); $$->filho = $3; $$->filho = criarIrmao($$->filho, $4);}
...
ParseArgs: PARSEINT OCURV ID OSQUARE Expr CSQUARE CCURV         {$$ = criarNo("ParseArgs", NULL); $$->filho = criarNo("Id", $3); $$->filho = criarIrmao($$->filho, $5);}
        | PARSEINT OCURV error CCURV                            {error = 1;}
        ;

Expr2:MethodInvocation                  {$$ = $1;}              /*E' -> € {E'.n = E'.i}*/             
    | ParseArgs                         {$$ = $1;}
    | Expr2 AND Expr2                   {$$ = criarNo("And", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);} /*AND vai para o topo da pilha, está no topo e a este vamos acrescentar um filho Expr e a este um irmão Expr*/
    | Expr2 OR Expr2                    {$$ = criarNo("Or", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 EQ Expr2                    {$$ = criarNo("Eq", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 GEQ Expr2                   {$$ = criarNo("Geq", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 GT Expr2                    {$$ = criarNo("Gt", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 LEQ Expr2                   {$$ = criarNo("Leq", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 LT Expr2                    {$$ = criarNo("Lt", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 NEQ Expr2                   {$$ = criarNo("Neq", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 PLUS Expr2                  {$$ = criarNo("Plus", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 MINUS Expr2                 {$$ = criarNo("Minus", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 STAR Expr2                  {$$ = criarNo("Star", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 DIV Expr2                   {$$ = criarNo("Div", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
    | Expr2 MOD Expr2                   {$$ = criarNo("Mod", NULL); $$->filho = $1; $$->filho = criarIrmao($$->filho, $3);}
